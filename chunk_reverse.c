...
#include<stdio.h>
    ...
    Node* chunk_reverse(Node **headAddress, int k){
        ...
        Node *nextNode = NULL;
        int n= 0; //counter to count till k nodes are reached
    ...
    //adding values through linked lists throug user
        ...
        printf("\nThe linked list is : \n");
        printList(head); //calls the print function to print the list
        ...
        do{
            if(k<=0)
            {
            printf("Invalid chunk size, please enter a positive number");
            flag=1; //sets flag to 1 to to repeat the loop
        ...
        if(k==0){
            ...
            Node *chunked = chunk_reverse(&head, k);
